...
Server::Server(EventLoop *loop, int threadNum, int port)//这里主要完成了两步，一是用loop初始化了线程池，二是初始化了acceptChannel
  ...
  m_connPool->init("localhost", "root1", "123456", "web", 3306, 50, 0);//线程池的sql数量和日志是否关闭
...
void Server::start() {
  ...
  acceptChannel_->setConnHandler(bind(&Server::handThisConn, this));//问题：这里为什么是handThisConn调用update？
...
void Server::handNewConn() {
  ...
  while ((accept_fd = accept(listenFd_, (struct sockaddr *)&client_addr,
    ...
    */
    ...
    req_info->getChannel()->setHolder(req_info);//便于从channel获取httpdata对象
